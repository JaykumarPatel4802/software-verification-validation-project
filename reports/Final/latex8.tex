...
While these approaches demonstrate the effectiveness of multi-agent pipelines for improving Text2SQL performance, there is value in systematically comparing different LLMs and framework designs under a constant experimental setting. In our work, we evaluate agentless and agent-based pipelines across multiple LLMs in order to better understand how model choice and pipeline structure influence SQL generation quality. Specifically, we evaluate not only a larger model such as Claude 3.5 Sonnet, but also smaller, potentially more economical models like GPT-4o Mini and Llama 3.1 8B. For feasibility, we employ a simplified version of the agent-based framework, utilizing a single generator rather than multiple specialized generators. To assess the consistency of each model and framework combination, we test each NLQ on each configuration three times.
