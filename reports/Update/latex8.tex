...
We explore 2 different frameworks, with (Agent-based) and without AI agents (Agentless), and 3 different LLMs, one of which is GPT-4o and the others are to-be-decided.

\section{Methodology}

In this section, we outline the methodology used for our differential analysis of LLMs in Text2SQL generation.

We will compare the performance difference between 2 frameworks, one that utilizes a series of AI agents (Agent-based), and one that performs without AI agents (Agentless).
\begin{enumerate}
...
\section{Results}
For this update, we have run the `Agentless' and `Agent-based' framework with GPT-4o on Chinook and BIRD-SQL. The Chinook benchmark includes 13 natural language queries and the BIRD-SQL includes 10 natural language queries (3 simple, 4 moderate, and 3 complex -- categorizations provided by BIRD-SQL).
